...
- `achievements.py` - 每日/每周成就总结
- `mood.py` - 心情计算与分档
- `hotkeys.py` - 全局热键管理（Windows API）
- `hotkey_hints.py` - 热键提示文案生成
- `binding_utils.py` - 模型动作/表情提取工具
- `launchers.py` - 快速启动器管理
- `plugins.py` - 插件系统管理/加载/钩子
- `bridge.py` - 扩展：插件API/备份恢复信号
---
...
#### 9.3 插件管理
- **查看** - 显示插件列表（名称/版本/状态/路径/错误信息）
- **操作** - 启用/禁用、重新加载、安装/卸载
- **日志** - 查看插件运行日志（data/plugins/<id>/plugin.log）

#### 9.4 插件开发指南
- **目录结构**：`plugins/<plugin_id>/` 目录下包含 `plugin.json` 和入口文件
